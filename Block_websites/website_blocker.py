...
while True:
    if dt(dt.now().year,dt.now().month,dt.now().day,8) < dt.now() < dt(dt.now().year,dt.now().month,dt.now().day,19):
        ...
        with open(hosts_path, 'r+') as file:
            content = file.readlines() #readlines creates a string for each line
            file.seek(0) #changes pointer to start
            for line in content:
                if not any(website in line for website in website_list):
                    file.write(line)
            file.truncate() #deletes any lines after the final line check
        print("fun hours...")
    time.sleep(5)
